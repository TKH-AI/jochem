...
TEST_FILE = '/projects/0/prjs1229/resources_x_4/test_dataset.csv'
MAX_NEW_TOKENS = 256
SUBSETS = ['2', '5', '7', '10', '12', '25', '50', '75', '100'] # Updated subsets
BATCH_SIZE = 64  # Added batch size constant
...
def get_model_paths():
    ...
    for subset in SUBSETS:
        distill_paths[subset] = find_checkpoint_dir(f"/projects/0/prjs1229/results_llama/distill/x/fourth_run/run_{subset}")
        vanilla_paths[subset] = find_checkpoint_dir(f"/projects/0/prjs1229/results_llama/vanilla/x/fourth_run/run_{subset}")
...
def setup_model_and_tokenizer():
    ...
    tokenizer.padding_side = 'left'  # Matches your training setup
    
    # Initialize quantization config - matching your training setup
    ...
    # Initialize base model - matching your training setup
    model = LlamaForCausalLM.from_pretrained(
        MODEL_NAME,
        quantization_config=quantization_config,
        device_map="cuda:0",  # Explicitly specify GPU device as in training
...
def generate_batch_text(model, tokenizer, prompts, batch_size=BATCH_SIZE, max_new_tokens=MAX_NEW_TOKENS):
    ...
    torch.manual_seed(45) #run 1 42, run 2 43, run 3 44, run 4 45
    torch.cuda.manual_seed(45) #run 1 42, run 2 43, run 3 44, run 4 45
...
def main():
    ...
    prompts = [
        f"Generate a marketing post on the platform X for the following datasheet description: {input_text} Marketing Post:"
    ...
    for subset in SUBSETS:
        ...
        distill_outputs = [
            output.split("Marketing Post:", 1)[1].strip() if "Marketing Post:" in output else output
    ...
    output_dir = '/projects/0/prjs1229/results_llama/generated_answers/x'
