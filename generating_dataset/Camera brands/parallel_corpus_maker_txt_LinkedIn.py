...
async def load_data_async(folder_prefix, item_type):
    ...
    for i in range(1, 7):
        ...
        if os.path.exists(datasheet_file) and os.path.exists(content_file):
            tasks.append(asyncio.gather(load_file(datasheet_file), load_file(content_file)))
    
    data = []
    for i, (datasheet, content) in enumerate(await asyncio.gather(*tasks)):
        data.append((f'datasheet_description_{i+1}.txt', datasheet.strip(), content.strip()))
    return data
...
async def generate_content_batch(descriptions, data, content_type, start_index, indices, executor):
    ...
    for i, (description, idx) in enumerate(zip(descriptions, indices)):  # Now we also track index in the original DataFrame
        ...
        if content_type == "LinkedIn":
            full_prompt = (
                "You are a content creator generating LinkedIn posts for various camera models. "
                "Based on the six provided examples, generate a new LinkedIn post for the following camera model description. "
                "**Do not include any introductory sentences like 'Here is a LinkedIn post for camera'. Only output the LinkedIn post content.**\n\n"
                f"**Examples:**\n{examples}\n\n"
                "Now, focus on the **new camera model** in the Datasheet Description below and create a LinkedIn post specifically for this model, not for any of the examples.\n\n"
                f"**New Camera Model:**\nDatasheet Description: {description}\nLinkedIn Post:"
            )
        else:
            full_prompt = (
                "You are a content creator tasked with explaining the reasoning behind LinkedIn posts based on the camera datasheet description. "
                "Based on the six provided examples, generate a new rationale for the following camera model description. "
                "**Do not include any introductory sentences like 'Here is a rationale for the LinkedIn post'. Only output the rationale content.**\n\n"
                f"**Examples:**\n{examples}\n\n"
...
async def process_linkedin_posts():
    await process_batch(df['datasheet_description'].tolist(), linkedin_data, 'LinkedIn', 'LinkedIn')

# File paths
input_file_path = '/Users/jochem/Desktop/mijn_project2/Camera brands/Final Dataset REAL ONES/dataset_LinkedIn_LLaMa8.csv'
output_file_path = '/Users/jochem/Desktop/mijn_project2/Camera brands/Final Dataset REAL ONES/dataset_LinkedIn_LLaMa9.csv'
