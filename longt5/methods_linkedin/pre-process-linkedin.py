...
file_path = '/Users/jochem/Desktop/thesis-code-methods/resources_linkedin/final_linkedin_clean_llama.csv'
df = pd.read_csv(file_path)

# Vanilla dataset preparation
df_vanilla = df[['datasheet_description', 'LinkedIn']]
...
df_distill = df[['datasheet_description', 'LinkedIn', 'Rationale']]
...
train_valid_split = dataset_combined.train_test_split(test_size=0.2, seed=42)
...
output_dir = '/Users/jochem/Desktop/thesis-code-methods/resources_linkedin_1/'
...
def save_train_subset(dataset, folder_path, subset_name, subset_percentage):
    subset_train_dataset = dataset['train'].shuffle(seed=0).select(range(int(len(dataset['train']) * subset_percentage)))
